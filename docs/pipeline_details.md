...
than just run it. For the mathematical formulation of the model, see
Ceseri, Natalini and Pezzella, *SIAM J. Appl. Math.* 85(6): 2591–2610,
[10.1137/24M1709704](https://doi.org/10.1137/24M1709704); for the calibration
strategy and discussion of the results, see the associated manuscript and,
in particular, Sections S3.2 and S3.3 of its Supporting Information.
...
Solid arrows are the forward data flow. The dashed arrows indicate that both
solvers read the smoothed data directly rather than receiving them as an
argument, which is why `Smoothed_Data.mat` must exist before either simulation
can run.

Each stage checks whether its own output already exists on disk. If it does,
it is loaded; if it does not, the previous stage is invoked through `run` and
its figures are closed immediately. Running the last script on an empty
...
destroys variables that the caller still needs, and the resulting failures
...
| `mu` | L c<sup>ref</sup> eps<sub>g</sub><sup>ref</sup> | optical thickness of the layer |
| `nu` | eps<sub>c</sub><sup>ref</sup> / eps<sub>g</sub><sup>ref</sup> | ratio of CdS to CdSO<sub>4</sub> reference absorptivities |

These are the only quantities calibrated. Everything else — the illumination
spectrum, the absorptivity curves, and the water profile — enters as a fixed
dimensionless input function derived from measurements.
...
The CdSO₄ absorptivity is then represented as `eps_c_values/nu`, so that
`nu` controls the relative absorptivity of CdS and CdSO₄ and is calibrated
together with `xi` and `mu`. Both are evaluated on the physical wavelength
window [380, 512] nm mapped onto the dimensionless *λ* ∈ [0,1].
...
and each column is then shifted so that all columns begin at their own first
positive value. The effect is to align the physical surface of the paint
across the map, compensating for the irregular profile of the embedded
section. The number of retained rows is the same for all columns, so the
result is still a rectangular matrix.
...
closed polygon used for the shaded uncertainty bands in the figures.
...
parameterizes smooth, strictly increasing positive profiles directly.
...
for a suitable auxiliary function *g*. Since

```
f'(z) = f'(z₀) exp( ∫₍z₀₎ᶻ g(s) ds ),
```

a positive initial derivative remains positive throughout the interval.
Smoothing therefore reduces to finding *g* instead of *f*. Here *g* is
expanded on shifted Legendre polynomials on the data interval,
...
orthogonal, the L² norm of *g* reduces to a weighted sum of squared
coefficients,
...
performed by `fmincon` with the interior-point algorithm, tolerances at
1e-14, and parallel gradients enabled.

Initial conditions are taken from the data: *f*(z₀) is the minimum of the
profile and *f*'(z₀) is the magnitude of the first finite difference.

Per-sample settings, which were tuned by inspection of the residuals against
...
deliberate, documented interventions on the two shallowest data points, not
a general filtering procedure.
...
comparable across samples with different degradation extents.

**Temporal weights.** `eta = [0.5, 1, 1.75, 1, 3]` reflect how much each
measurement is weighted in the calibration. Sample t10 carries triple
weight because its concentration change is well above the 5% instrumental
sensitivity threshold; t7 receives nearly double weight. Sample t1 is
halved because it shows CdS concentrations slightly *higher* than t4 and t7
despite being less exposed, an inconsistency attributed to experimental
inaccuracy.

**Spatial weights.** `theta_ij = i exp(-beta_i z_j)`, with *i* = 1..5 the
sampling time and *j* the depth index, concentrate the fit near the surface,
where photodegradation acts. In the code, the loop variable `j` runs over
the sampling times rather than over depth, so the same expression reads
`j*exp(-alpha(j)*(1:m)*0.25)` with
`alpha = [0.01, 1, 0.6, 0.5, 0.01]`. The factor 0.25 converts the depth
index into a depth coordinate in micrometres, so the corresponding published
values are `beta_i = 0.25*alpha_i`, that is

[2.5·10<sup>-3</sup>, 0.25, 0.15, 0.125, 2.5·10<sup>-3</sup>].

**Optimization.** The procedure has two stages: `simulannealbnd` explores
the admissible box globally, then its result seeds an interior-point
`fmincon` refinement. The published optimum is

```
p* = [xi*, mu*, nu*] = [3.023, 5.470, 0.764]
```

and the calibration errors remain within the 5% experimental sensitivity
threshold over the calibration dataset, for both the smoothed profiles used
in the fit and the original y-averaged data.
...
> optimum `mu* = 5.470` therefore lies at the numerical upper bound used by
> the code, whereas the manuscript reports an upper bound of 6. This is an
> implementation detail that differs from the bound reported in the
> manuscript. The hard-coded values are part of the calibration as performed
> and are deliberately left untouched; anyone repeating the calibration
> should be aware that this component is bound-constrained.
...
Both solvers use a second-order predictor–corrector method based on
...
2. **Corrector** — the spectral integral is recomputed with the trapezoidal
   rule using the predictor.
...
The offset in the reference time is also deliberate. The calibration
solver uses a value of `T` slightly larger than 680 h so that the resulting
`D_t` makes the MATLAB expression `0:D_t:1` produce the intended 681 grid
points and the loop reach `n = 680`, preserving the t10 level, which carries
the highest temporal weight. The long-term solver uses the analogous `+1`
second offset to obtain the intended number of daily time points.

The calibration solver stores its solution at `n = 24, 80, 167, 340, 680`,
corresponding to the five experimental exposures in hours. The long-term
solver returns the full space-time matrix instead, because the downstream
rescaling requires arbitrary columns.
...
concentration cannot exceed the initial value. This is purely for visual
...
(10951, corresponding to day 10,950) giving the 30-year prediction. The
result is a depth-dependent evolution factor expressing how much further
each sample would degrade from the state at which it was measured.
...
Thus lateral structure comes from the measurement and vertical evolution
from the model. The final figure shows measured maps on the first row and
...
feed the CdSO4 summary table printed at the end of the script, which reports
for each sample the degradation-front position at one and 30 years, the
mean CdSO4 concentration between the surface and the front, and the
percentage increase of the total content. Because the rescaling factor is
constant along *y*, the table is computed from `mean(M,2)` and `sum(M,2)`
of each measured map rather than from explicitly constructed rescaled maps,
using
...
Both identities are exact, and they avoid allocating ten further arrays the
size of the dense maps.
...
* The four MAT files are committed, about 3 MB in total. Each stage saves
  ...
  holds only the ten rescaling factors; the 15101 × 10951 solution and the
