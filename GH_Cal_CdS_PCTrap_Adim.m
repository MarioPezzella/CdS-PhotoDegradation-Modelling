function [Sol] = GH_Cal_CdS_PCTrap_Adim(xi,mu,nu)
% =========================================================================
% GH_CAL_CDS_PCTRAP_ADIM
%
...
%
% OUTPUT:
%   Sol - CdS relative concentration profiles at selected times
...
T = 2448003;                % Reference time [s] (680 h)
...
% =========================================================================
D_z = 0.006622516556291;    D_t = 3600/T;   D_L = 1e-2;
z = 0:D_z:1;    t = 0:D_t:1;    L = 0:D_L:1;
N_t = length(t);    N_z = length(z);
pred = ones(N_z,1); C = ones(N_z,N_t);  Sol = zeros(N_z,5);
...
for n = 1:N_t-1
    ...
    % =====================================================================
    % Store solution at the experimental time levels
    % =====================================================================
    if n == 24 || n == 80 || n == 167 || n == 340 || n == 680
        % Hours needed for the comparison with experimental samples
        Sol(:,time_lvl) = C(:,n+1);
        time_lvl = time_lvl + 1;
    end
end
